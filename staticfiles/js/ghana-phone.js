...
(function () {
  ...
  function nationalDigits(value) {
     ...
     * pasted "+233241234567" and legacy "0241234567", then cap at 9. A
     * leading zero never survives — Ghanaian mobile prefixes never start
     * with one, so it is dropped the instant it is typed or pasted and the
     * remaining digits shift in, keeping the box at the 9 digits after +233. */
  ...
  function initGhanaPhoneField(input) {
    ...
    // Normalise pasted text before maxlength can truncate it: the legacy
    // "0241234567" would otherwise lose its last digit to the 9-character
    // cap before the leading zero could be stripped.
    input.addEventListener("paste", function (ev) {
      var text = ev.clipboardData ? ev.clipboardData.getData("text") : "";
      if (!text) { return; }
      ev.preventDefault();
      input.value = nationalDigits(text);
      clearError();
      paint();
    });
